...
def beam_decode(batch_size, SOS_token, EOS_token, decoder_hiddens, decoder, attention_mod, encoder_outputs, decoder_inputs, embedding):
    '''
    ...
    for idx in range(batch_size):
        ...
        while True:
            ...
            if qsize > 1:
                decoder_input = torch.cat((aud_context, embedding(decoder_input).unsqueeze(1)), dim=2) 
            elif qsize == 1:
                decoder_input = torch.cat(decoder_inputs[idx, ...].unsqueeze(0),
                    embedding(decoder_input).unsqueeze(1), dim=2)
