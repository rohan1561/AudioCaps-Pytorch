...
class MultimodalAtt2(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, dim_hidden, dim_word, dim_vid=2048, 
        ...
        super(MultimodalAtt2, self).__init__()
        ...
        if mode == 'train':
            ...
            for _ in range(self.max_len-1):
                ...
                for i in range(len(current_words)):
                    s = current_words[i]
                    emb_words = self.embedding(s[0][-1]) # ?? -1 or 0?(both same?)
                    self.decoder.flatten_parameters()
                    if i == 0:
                    ...
                    decoder_output, s[2] =\
                            self.decoder(decoder_input, s[2])
                    ...
                    for probs, words in zip(topk_probs, topk_words):
                        next_cap = s[0][:]
                        next_cap.append(words) # words.shape = 128
                        temp.append([next_cap, s[1]+probs, (s[2][0].clone(), s[2][1].clone())])
                ...
                m = 0
                for word in temp:
                    layer_probs.append(word[1].unsqueeze(1))
                    layer_words.append(word[0][-1].unsqueeze(1))
                    full_sentence[m] = torch.stack(word[0], dim=1) # 128, len(decoded)
                    m += 1
                layer_probs = torch.stack(layer_probs, dim=1) # 128, 5 or 128, 25
                layer_words = torch.stack(layer_words, dim=1) # 128, 5 or 128, 25
                print(layer_probs.shape, layer_words.shape)

                layer_topk_probs, indices = torch.topk(layer_probs, k=beam_size, dim=1)
                mask = torch.zeros_like(layer_words)
                mask.scatter_(1, indices, 1)

                layer_words = mask*layer_words
                layer_words, _ = torch.topk(layer_words, k=beam_size, dim=1) # 128, 5 Get only the words and lose the redundant zeros
                #layer_words = layer_words.permute(1, 0) # 5, 128

                for r in range(indices.shape[0]):
                    for j, k in zip(indices[r], layer_words[r]):
